 ...
 *
  ...
  @retval     The pointer to the new create HOB.
**/
...
/**
  ...
  @param[in]  HobListPtr     The starting memory address of HOB list.
  @param[out] DxeFv          The pointer to the DXE FV in memory.
  @retval EFI_SUCCESS        If it completed successfully.
  @retval Others             If it failed to build required HOBs.
**/
  ...
  )
{
  ...
  if (HobListPtr != 0) {
    ...
    if (FdtCheckHeader (FdtBase) == 0) {
      HobListPtr = CustomFdtNodeParser ((VOID *)FdtBase, (VOID *)HobListPtr);
    } else {
      CreatNewHobForHoblist (BootloaderParameter);
    }

    // Build HOB based on information from Bootloader
    Status = FitBuildHobs (HobListPtr, &DxeFv);
  }
